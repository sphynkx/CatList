...
class CatListHooks {
  ...
  */
  public static function wfCatList( $input, array $args, Parser $parser, PPFrame $frame ) {
	...
	## NOTE: if the tag body is empty, the category name defaults to the
	## CURRENT PAGE'S OWN TITLE (namespace prefix stripped) - this only
	## makes sense when the tag sits on the category page itself. On any
  ...
  *
  * FIXED: now uses an explicit join_conds array instead of a raw WHERE-
  * clause implicit join (the old "cl_from = page_id" string, with an
  * unqualified page_id column, was fragile across DB abstraction/schema
  * differences and could silently return zero rows instead of erroring).
  * Also prefers the modern getConnectionProvider() (MW 1.42+) and falls
  * back to getDBLoadBalancer() on older cores, so the same code works on
  ...
  */
  ...
  FIXED: null-guards throughout - Title::newFromText() and getContent()
  can both legitimately return null (invalid/nonexistent title, missing
  revision, restricted/deleted content), and every preg_match() result was
  being indexed with [1] without checking a match was actually found. Any
  one of these used to be a fatal "call to member function on null" /
  "undefined array key" - now they degrade gracefully to empty values
  ...
  */
  public static function getThumbItem($pageTitle, $nameSpace){
	...
	$synVal = isset($syn[1]) ? preg_replace('/.*:/', '', $syn[1]) : ''; ## preserve affect to interwiki mrchanizms
	$syn = (strlen($synVal) > 2) ? '<hr>([['. preg_replace('/;\s?/', ']]; [[', trim($synVal)) . ']])' : '';

	preg_match('/^.*?\|\s*?НатСинонимы\s*?=(.*?)\|/u', $wikitext, $natsyn);
	$natsynVal = isset($natsyn[1]) ? preg_replace('/.*:/', '', $natsyn[1]) : ''; ## preserve affect to interwiki mechanizms
	$natsyn = (strlen($natsynVal) > 2) ? '<hr>([['. preg_replace('/;\s?/', ']]; [[', trim($natsynVal)) . ']])' : '';
	...
	$img = (isset($img[2]) && strlen($img[2]) > 2) ? '[[File:' . trim($img[2]) : '[[File:Unknown-person.png';
...
EOD;
  ...
  */
  public static function getSubtitlesThumbItem($pageTitle, $nameSpace, $wikitext){
	...
	$pict = trim($tpl_match[1] ?? '');
	$title_attr = trim($tpl_match[2] ?? '');
...
[[$nameSpace$pageTitle|
<html><img src=$pict width="$max_width" title="$title_attr">
...
EOD;
  ...
  *
  * FIXED: same null-guard treatment as getThumbItem() - this is exactly
  * the function/line that produced the "Call to a member function
  * getNamespace() on null" fatal (Title::newFromText() returning null,
  * fed unchecked into getRevisionByTitle()). Also fixed a real PHP7-vs-8
  * landmine: `$x > 0` on a category-name STRING compares as a number
  * pre-PHP8 (non-numeric string -> 0, so 0>0 is always false - silently
  * dropping every category name) and as a string post-PHP8 (almost
  * always true) - replaced with an explicit non-empty check that means
  * the same thing on every PHP version.
  */
  public static function getAddedCategories($pageTitle, $nameSpace, $catPages){
	...
	$add_themes = isset($add_themes[1]) ? explode("\n", $add_themes[1]) : [];
	...
	$cats[1] = array_filter($cats[1], function($x){ return is_string($x) && trim($x) !== ''; });

	## Fixed: Raised error (null array item) if category page exists but same name page doesnt exist
	$getcats = array_filter($cats[1], function($x){
	  $t1 = Title::newFromText($x);
	  $t2 = Title::newFromText('Category:'.$x);
	  return ($t1 && $t1->getLatestRevID() > 0) || ($t2 && $t2->getLatestRevID() > 0);
	});
